...
from .serializers import EntidadeSerializer, UtilizadorSerializer, CarroSerializer, AnexosSerializer
import json
...
@api_view(['POST'])
def LoginUtilizadorView(request):
    data = request.data
...
@permission_classes([AllowAny])    
@api_view(['POST'])
def RegistarUtilizadorView(request):
    try:
...
@permission_classes([AllowAny])
@api_view(['POST']) 
def RegistarEntidadeView(request):
        data = json.loads(request.body)
